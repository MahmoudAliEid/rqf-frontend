...
type Blog = { images: string[]; id: string ,title: string,ar_title: string, content: string,ar_content:string};
...
const BlogList = () => {
    ...
    queryFn: async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_BACKEND_URL}/api/all-blogs`,
          {
            method: "GET",
            headers: {
  ...
  const { mutate: deleteImage } = useMutation({
    mutationFn: async (id: string) => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_BACKEND_URL}/api/delete-blog/${id}`,
          {
            method: "DELETE",
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        if (!response.ok) {
          ...
          throw new Error(`HTTP ${response.status}: Failed to delete blog`);
        }
  ...
  const handleDelete = (id: string) => {
    if (!id) {
      ...
      toast({
        title: "خطأ",
        description: "معرف المدونة غير صحيح",
        variant: "destructive",
      });
      return;
    }
                ...
                <div
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 sm:gap-8 items-center w-full sm:w-auto">
                    ...
                    <div className="text-center sm:text-right">
                      <h3 className="text-lg md:text-xl font-semibold">
                         {
                        i18n.language === "ar" ? product.ar_title : product.title || "منتج بدون اسم"
                       }
                      </h3>
                      <p className="text-gray-500 text-sm md:text-base">
                        {i18n.language === "ar" ? product.ar_content : product.content || "لا يوجد وصف"}
                      </p>
                    </div>
                  </div>
                  ...
                  onClick={
                    ()=>{
                      ...
                      navigate(`/admin/edit-blog/${product.id}`);
                    }
                  }>

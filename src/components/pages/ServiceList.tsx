...
type Service = { images: string[]; id: string ,name : string,ar_name: string, description: string,ar_description:string};
...
const ServicesList = () => {
    ...
    queryFn: async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_BACKEND_URL}/api/all-services`,
          {
            method: "GET",
            headers: {
  ...
  const { mutate: deleteImage } = useMutation({
    mutationFn: async (id: string) => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_BACKEND_URL}/api/delete-service/${id}`,
          {
            method: "DELETE",
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        if (!response.ok) {
          ...
          throw new Error(`HTTP ${response.status}: Failed to delete service`);
        }
  ...
  const handleDelete = (id: string) => {
    if (!id) {
      ...
      toast({
        title: "خطأ",
        description: "معرف الخدمة غير صحيح",
        variant: "destructive",
      });
      return;
    }
                ...
                <div
                  ...
                  onClick={
                    ()=>{
                      ...
                      navigate(`/admin/edit-service/${product.id}`);
                    }
                  }>
